...
// Função otimizada para abrir WhatsApp (App ou Web)
window.openWhatsApp = (phone, name) => {
    const message = `Olá ${name}! Seu teste está liberado.`;
    const encodedMessage = encodeURIComponent(message);
    const fullPhone = phone.startsWith('55') ? phone : `55${phone}`;
    
    const waApp = `whatsapp://send?phone=${fullPhone}&text=${encodedMessage}`;
    const waWeb = `https://wa.me/${fullPhone}?text=${encodedMessage}`;

    // Tenta abrir o app diretamente
    window.location.href = waApp;

    // Fallback para web, caso o app não abra em 500ms
    setTimeout(() => {
        if (document.hasFocus()) { // Se ainda estiver na página, provavelmente o app não abriu
...
window.deleteClient = (id) => {
    if (confirm('Deseja realmente excluir este cliente? (Marcar como "Assinou")')) {
    ...
    return `
        <div class="bg-white p-5 rounded-[2.5rem] shadow-sm border transition-all animate-in ${isOverdue ? 'border-red-500 bg-red-50 shadow-red-100 shadow-xl' : 'border-slate-100'}">
            ...
            <div class="flex gap-2 mt-4">
                ...
                `}
                <button onclick="openWhatsApp('${client.phone}', '${client.name}')" class="w-14 h-14 bg-emerald-100 text-emerald-600 flex items-center justify-center rounded-3xl active:scale-90 transition-all">
